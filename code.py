...
client = MongoClient(
    "mongodb+srv://mukeshvijay26:n3qm15sX9MQnif26@cluster0.0jiak6b.mongodb.net/?retryWrites=true&w=majority")
db = client['database']
        ...
        with tab2:
            ...
            if st.button("Submit"):
                try:
                    ...
                    if existing_channel:
                        # If the channel exists, update the data
                        ch_details = get_channel_details(ch_id)
                        existing_channel_id = existing_channel[0]  # Get the channel ID from the result
                        ch_details = get_channel_details(existing_channel_id)  # Use existing_channel_id as the channel ID

                        # Retrieve video IDs for the existing channel
                        v_ids = get_channel_videos(existing_channel_id, max_results)  # Pass max_results to the function

                        vid_details = get_video_details(v_ids, max_results)  # Pass max_results to the function
                        comm_details = comments()
                        playlist_data = get_playlist_details(existing_channel_id, max_results)  # Use existing_channel_id as the channel ID

                        # Update the data in MySQL
                        update_channels(ch_details)  # Pass the channel details
                        update_videos(vid_details)  # Pass the video details
                        update_comments(comm_details)  # Pass the comment details
                        update_playlists(playlist_data)  # Pass the playlist details
                        st.success("Data for the selected channel updated successfully in MySQL!")
                    else:
...
if selected == "About":
    ...
    st.markdown(
        "### :green[About the app] : This Streamlit app is designed to analyze YouTube data using Python and various libraries. It allows users to search for YouTube channels, transfer data to MySQL, and view insights about the data.")
    st.markdown(
        "### :green[About the creator] : This app was created by MK Jose, a data science enthusiast, and Python developer.")